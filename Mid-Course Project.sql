...
2)	Next, it would be great to see a similar monthly trend for Gsearch, but this time splitting out nonbrand and brand campaigns separately. I am wondering if brand is picking up at all. 
...
4)	I am worried that one of our more pessimistic board members may be concerned about the large % of traffic from Gsearch. Can you pull monthly trends for Gsearch, alongside montly trends for each of our other channels?
SELECT
    YEAR(website_sessions.created_at) AS year,
	MONTH(website_sessions.created_at) AS month,
...
5)	Could you pull session to order conversión rates, by month? I’d like to tell the story of our website performance improvements over the course of the first 8 months.
SELECT
	YEAR(website_sessions.created_at) AS year,
	MONTH(website_sessions.created_at) AS month,
	COUNT(DISTINCT website_sessions.website_session_id) AS sessions,
...
-- I will first find the first pageview_id
...
-- to see each first page view id per each session_id
SELECT
...
FROM website_pageviews
	INNER JOIN website_sessions
        ...
        AND website_sessions.created_at < '2012-07-28' -- prescribed by the assigment
...
-- to find the differencee between conversion rates
SELECT
	landing_page,
...
-- 22972 website sessions since the rest
...
7)	For the landing page test you analyzed previously, please show a full conversión funnel from earch of the two pages to orders. It can be used the same time period (Jun 19 – Jul 28)
...
-- below query produces the final output, part 1
...
-- here goes below final output, part 2 - click rates
...
FROM sessions_level_made_it_flagged
GROUP BY 1;
8)	Please quantify the impact of our billing test, as well. Please analyze the lift generated form the test (Sep 10 – Nov 10), in terms of revenue per billing page sesión, and then pull the number of billing page sessions for the past month to understand montly impact.
SELECT
...
FROM website_pageviews
	...
	WHERE website_pageviews.created_at > '2012-09-10' -- prescribed in assigment
		AND website_pageviews.created_at < '2012-11-10' -- prescribed in assigment
